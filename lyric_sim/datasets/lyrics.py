...
class LyricsSqliteDataset(Dataset):
    def __init__(self, lyrics_db_path: str, mxm_db_path: str, lastfm_db_path: str, sim_csv_path: str,
                 use_test: bool, pad_length: bool):
        ...
        # read in lastfm ids
        qry = 'SELECT tid FROM similars_src'
        df_lastfm = pd.read_sql_query(qry, con_lastfm)

        # similarity score (from lastfm)
        df_sim_all = pd.read_csv(sim_csv_path)
        ...
        df = df.merge(df_lastfm, how='inner', on='tid')
        ...
        df_sim_all = df_sim_all.merge(df_ids, how='left', left_on='Track1', right_on='tid')
        df_sim_all = df_sim_all.rename(columns={'index': 'track1_id'})
        df_sim_all = df_sim_all.merge(df_ids, how='left', left_on='Track2', right_on='tid')
        df_sim_all = df_sim_all.rename(columns={'index': 'track2_id'})
        
        # keep if both tracks have lyrics
        df_sim_lyrics = df_sim_all[(~df_sim_all['track1_id'].isnull()) & (~df_sim_all['track2_id'].isnull())].copy()
        
        # set 1/0 for similarity
        df_sim_lyrics['similar'] = np.where(df_sim_lyrics['Similarity'] >= SIM_THRESHOLD, 1, 0)
    ...
    def __getitem__(self, index):
        ...
        return ((self.data[int(self.df_sim.loc[index, 'track1_id'])], self.data[int(self.df_sim.loc[index, 'track2_id'])]), self.df_sim.loc[index, 'similar'])